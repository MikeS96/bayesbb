...
def train(elbo_samples: int, batch_size: int, num_epochs: int, lr: float,
    ...
    num_classes = 10
    if model_param == "det":
        bl_mnist_model = BaselineMnist().to(device)
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(bl_mnist_model.parameters(), lr=0.01)
